...
@section('content')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Fungsi untuk menghitung waktu selesai berdasarkan durasi
            function calculateEndTime() {
                ...
                if (startTime && duration) {
                    ...
                    // Validasi waktu selesai tidak melebihi 23:00
                    if (endHours <= 23) {
                        const formattedEndHours = String(endHours).padStart(2, '0');
                        endTimeInput.value = `${formattedEndHours}:${String(minutes).padStart(2, '0')}`;
                    } else {
                        // Jika melebihi 23:00, set ke 23:00 dan sesuaikan durasi
                        endTimeInput.value = '23:00';
                        // Opsional: beri peringatan
                        // alert('Waktu selesai melebihi batas maksimal 23:00');
                    }
                } else {
                    endTimeInput.value = '';
                }
            ...
            // Fungsi untuk mengatur durasi default
            function setDefaultDuration() {
                if (durationSelect && !durationSelect.value) {
                    // Cek apakah ada data waktu mulai dari query string atau old input
                    const hasStartTime = startTimeInput && startTimeInput.value;
                    
                    if (hasStartTime) {
                        // Set durasi default ke 1 jam
            ...
            if (durationSelect) {
                ...
                // Set durasi default saat halaman dimuat
                setTimeout(setDefaultDuration, 100);
            }
            
            // Inisialisasi perhitungan waktu selesai
            if (startTimeInput && durationSelect && endTimeInput) {
                // Jika sudah ada data dari awal, hitung waktu selesai
